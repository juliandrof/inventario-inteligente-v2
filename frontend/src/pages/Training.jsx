...
import {
  ...
  fetchConfigFixtureTypes, fetchServingEndpoints, updateConfig, fetchConfigs,
  uploadVideo,
} from '../api';
...
const TABS = [
  { key: 'upload', label: 'Upload & Inferencia' },
...
/* ========== UPLOAD & INFERENCE WIZARD TAB ========== */
function UploadWizardTab() {
  const [step, setStep] = useState(1);
  const [files, setFiles] = useState([]);
  const [detectionMode, setDetectionModeState] = useState('LLM');
  ...
  // Load serving endpoints and trained models on mount
  useEffect(() => {
    ...
    Promise.all([
      ...
      const mode = (dm.mode || 'LLM').toUpperCase();
      setDetectionModeState(mode);
      // Pre-select current LLM from config
      updateConfig('fmapi_model', '').catch(() => {}); // just to trigger - ignore
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      // Step C: Upload and process files
      const uploadResults = [];
      for (let i = 0; i < files.length; i++) {
        setUploadProgress(`Enviando ${files[i].name} (${i + 1}/${files.length})...`);
        try {
          const result = await uploadVideo(files[i]);
          uploadResults.push({ file: files[i].name, success: true, result });
        } catch (err) {
          uploadResults.push({ file: files[i].name, success: false, error: err.message });
        }
      }
  ...
  const DETECTION_MODES = [
    ...
    { key: 'YOLO', label: 'YOLO', icon: '⚡', desc: 'Modelo YOLO treinado com seus dados. Deteccao rapida e precisa em tempo real.' },
    { key: 'HYBRID', label: 'Hibrido', icon: '🔄', desc: 'YOLO detecta bounding boxes + LLM classifica e descreve cada expositor.' },
  ];

  const activeYoloModel = models.find(m => m.is_active);
  const hasYoloModel = models.length > 0;
  ...
  return (
    <>
      ...
      {step === 1 && (
        <div className="card">
          <h3>Selecione videos ou fotos para analise</h3>
          <div
            className="upload-zone"
            onDrop={handleFileDrop}
            onDragOver={e => e.preventDefault()}
            onClick={() => fileInputRef.current?.click()}
            style={{ cursor: 'pointer', padding: 40, textAlign: 'center', border: '2px dashed #D1D5DB', borderRadius: 12, marginBottom: 16 }}
          >
            <div style={{ fontSize: 40, marginBottom: 8 }}>📁</div>
            <div style={{ fontSize: 14, color: '#6B7280' }}>Arraste videos ou fotos aqui, ou clique para selecionar</div>
            <div style={{ fontSize: 12, color: '#9CA3AF', marginTop: 4 }}>Formatos: MP4, AVI, MOV, JPG, PNG</div>
            <input
              ref={fileInputRef}
              type="file"
              multiple
          ...
          {files.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>{files.length} arquivo(s) selecionado(s):</div>
              {files.map((f, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: '1px solid #F3F4F6' }}>
                  ...
                  <button className="btn btn-sm btn-danger" onClick={() => removeFile(i)} style={{ padding: '2px 8px' }}>X</button>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <button className="btn btn-primary" onClick={goToStep2} disabled={files.length === 0}>
              Proximo →
            </button>
          </div>
        </div>
      ...
      {step === 2 && (
        <>
          <div className="card">
            ...
            <div className="detection-mode-grid">
              {DETECTION_MODES.map(mode => {
                const disabled = (mode.key === 'YOLO' || mode.key === 'HYBRID') && !hasYoloModel;
                return (
                  <div
                    key={mode.key}
                    className={`detection-mode-card ${detectionMode === mode.key ? 'detection-mode-card-active' : ''}`}
                    onClick={() => !disabled && setDetectionModeState(mode.key)}
                  ...
                  >
                    ...
                    {mode.key === 'YOLO' && activeYoloModel && (
                      <div style={{ fontSize: 11, color: '#10B981', marginTop: 4 }}>Modelo ativo: {activeYoloModel.model_name}</div>
                    )}
                  </div>
                );
              })}
          ...
          {/* LLM Model selection (for LLM and HYBRID modes) */}
          {(detectionMode === 'LLM' || detectionMode === 'HYBRID') && (
            <div className="card">
              <h3>Escolha o modelo LLM</h3>
              <p style={{ fontSize: 13, color: '#6B7280', margin: '0 0 16px' }}>
                Lista de modelos multimodais disponiveis no seu workspace Databricks.
              </p>
              {loadingEndpoints ? (
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0 0 16px' }}>
            ...
            <button
              ...
              disabled={(detectionMode === 'LLM' || detectionMode === 'HYBRID') && !selectedLLM}
            >
        ...
        <div className="card" style={{ textAlign: 'center', padding: 40 }}>
          {uploading && (
            <>
              ...
              <div style={{ width: '100%', height: 4, background: '#E5E7EB', borderRadius: 2, marginTop: 16 }}>
                <div style={{ width: '50%', height: '100%', background: 'var(--app-primary)', borderRadius: 2, animation: 'pulse 1.5s infinite' }} />
              </div>
            </>
          )}
              ...
              <div style={{ textAlign: 'left', maxWidth: 500, margin: '0 auto' }}>
                {results.map((r, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', borderBottom: '1px solid #F3F4F6' }}>
                    ...
                    <span style={{ fontSize: 13, flex: 1 }}>{r.file}</span>
                    {!r.success && <span style={{ fontSize: 11, color: '#EF4444' }}>{r.error}</span>}
                  </div>
                ))}
              </div>
              <button className="btn btn-primary" onClick={resetWizard} style={{ marginTop: 24 }}>
                Novo Upload
              </button>
            </>
          )}
      ...
      {/* YOLO Models management (always visible at bottom of step 2) */}
      {step === 2 && models.length > 0 && (
        <div className="card">
          <h3>Modelos YOLO Treinados</h3>
          <table className="data-table">
            <thead>
              <tr><th>Nome</th><th>mAP@50</th><th>Precision</th><th>Recall</th><th>Status</th><th>Acoes</th></tr>
            </thead>
            <tbody>
              {models.map(model => (
                <tr key={model.model_id}>
                  ...
                  <td>{model.precision_val != null ? (model.precision_val * 100).toFixed(1) + '%' : model.precision != null ? (model.precision * 100).toFixed(1) + '%' : '-'}</td>
                  <td>{model.recall_val != null ? (model.recall_val * 100).toFixed(1) + '%' : model.recall != null ? (model.recall * 100).toFixed(1) + '%' : '-'}</td>
                  <td>
                    {model.is_active
                  ...
                  <td>
                    {!model.is_active && (
                      <button className="btn btn-sm btn-primary" onClick={async () => { await activateModel(model.model_id); const m = await fetchTrainedModels(); setModels(m.models || m || []); }}>Ativar</button>
                    )}
                  </td>
                </tr>
              ))}
